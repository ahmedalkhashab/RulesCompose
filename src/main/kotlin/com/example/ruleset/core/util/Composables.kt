...
import com.example.ruleset.core.ComposeKtConfig
...
fun KtFunction.emitsContent(config: ComposeKtConfig): Boolean =
    when {
        isComposable ->
            findChildrenByClass<KtCallExpression> { current ->
...
private fun KtExpression.uiEmitterCount(config: ComposeKtConfig): Int {
    ...
    val emitterCount =
        DeepRecursiveFunction<KtExpression?, Int> { current ->
            when (current) {
                null -> 0
...
private fun KtFunction.indirectUiEmitterCount(
    mapping: Map<KtFunction, Int>,
...
fun Sequence<KtFunction>.createDirectComposableToEmissionCountMapping(
    config: ComposeKtConfig,
): Map<KtFunction, Int> = associateWith { it.uiEmitterCount(config) }
...
fun refineComposableToEmissionCountMapping(
    ...
    while (shouldMakeAnotherPass) {
        val updatedMapping =
            current.mapValues { (functionNode, _) ->
...
val KtProperty.declaresCompositionLocal: Boolean
    get() =
        !isVar &&
            ...
            (initializer as KtCallExpression).referenceExpression()?.text in
            CompositionLocalReferenceExpressions
