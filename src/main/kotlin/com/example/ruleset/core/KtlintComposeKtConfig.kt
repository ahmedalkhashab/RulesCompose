...
import com.example.ruleset.core.util.toSnakeCase
...
internal class KtlintComposeKtConfig(
    ...
    private fun <T : Any> getValueAsOrPut(
        key: String,
        value: () -> T?,
    ): T? =
        cache.getOrPut(key) {
    ...
    override fun getInt(
        key: String,
        default: Int,
    ): Int = getValueAsOrPut(key) { find<String>(key)?.toInt() } ?: default

    override fun getString(
        key: String,
        default: String?,
    ): String? = getValueAsOrPut(key) { find(key) } ?: default
    ...
    override fun getSet(
        key: String,
        default: Set<String>,
    ): Set<String> = getValueAsOrPut(key) { getList(key, default.toList()).toSet() } ?: default

    override fun getBoolean(
        key: String,
        default: Boolean,
    ): Boolean = getValueAsOrPut(key) { find(key) } ?: default
